	...
	const handleDeleteBuyProduct = (_id) => {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				fetch(
					`https://joy-with-toy-server.vercel.app/BuyToysInfo/${_id}`,
					{
						method: "DELETE",
					}
				)
					...
					.then((data) => {
						...
						if (data.deletedCount > 0) {
							...
							const remaining = byNowToyProducts.filter(
								(toys) => toys._id != _id
							);
				...
				<div className="card-body font-extrabold">
					...
					<div className="flex justify-between mt-6">
						<div className="card-actions justify-start">
							<button className="btn btn-outline btn-primary font-extrabold">
								Buy Now
							</button>
						</div>
						...
						<div className="card-actions justify-end">
							<button
								onClick={() => handleDeleteBuyProduct(_id)}
								className="btn btn-outline btn-error font-extrabold"
							>
								Cancel Order
							</button>
						</div>
